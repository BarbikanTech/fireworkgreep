    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($row["img"] != null && $row["img"] != 'null' && strlen($row["img"]) > 0) {
                $imgLink = "https://" . $_SERVER['SERVER_NAME'] . "/crackers/dlscrackers/uploads/banner/" . $row["img"];
        ...
        if (numericCheck($current_user_id)) {
            ...
            if (!empty($current_user_name)) {
                if (!empty($image_url)) {
                    ...
                    $createBanner = "INSERT INTO `banner`(`img`, `delete_at`, `created_by`, `created_name`, `created_date`) VALUES ('$profile_path',0,'$current_user_id','$current_user_name ','$timestamp')";
