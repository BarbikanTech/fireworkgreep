...
if (isset($obj->search_text)) {
    ...
    $sql = "SELECT * FROM `invoice` WHERE (invoice_no LIKE '%$userid%' OR customer_name LIKE '%$userid%' OR city LIKE '%$userid%' OR state LIKE '%$userid%') AND `deleted_at` = 0 ORDER BY `id` DESC";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $output["body"]["invoice"][$count]["created_date"] = date('d/m/Y', $timestamp);
            $count++;
        }
    } else {
        $output["head"]["code"] = 200;
